...
if(isset($_POST['submit'])){
	...
	$sql = 'select * from rjcms_user where u_name = "'.$u_name.'" and u_password = "'.md5($u_password).'" and u_status=1';
	...
	mysqli_query($conn, 'update rjcms_user set '.arrtoupdate($_data).' where u_id ="'.$row['u_id'].'"');
...
// 检测用户名是否存在
$query = mysqli_query($conn, "select u_id from rjcms_user where u_name='$username'");
if(mysqli_fetch_array($query)){
echo '<script>alert("用户名已存在，请换个其他的用户名");window.history.go(-1);</script>';
exit;
}
$result = mysqli_query($conn, 'select * from rjcms_user where u_email = "'.$_POST['email'].'"');
...
if (isset($_GET['tg'])) {
	$data['u_qid'] =$_GET['tg'];
	$result = mysqli_query($conn, 'select * from rjcms_user where u_id="'.$_GET['tg'].'"');
if($row = mysqli_fetch_array($result)){

$u_points=$row['u_points'];
}
	}
$_data['u_points'] =$u_points+$rjcms_tuiguang;
$sql = 'update rjcms_user set '.arrtoupdate($_data).' where u_id="'.$_GET['tg'].'"';
if (mysqli_query($conn, $sql)) {}	
$data['u_question'] =$token;
$str = arrtoinsert($data);
$sql = 'insert into rjcms_user ('.$str[0].') values ('.$str[1].')';
if (mysqli_query($conn, $sql)) {
if($rjcms_mail==1){
//写入数据库成功，发邮件
include("emailconfig.php");
    ...
    $email = $email;  //收件人邮箱
    $emailTitle = "".$rjcms_name."用户帐号激活"; //邮件主题
    $emailBody = "亲爱的".$username."：<br/>感谢您在我站注册了新帐号。<br/>请点击链接激活您的帐号。<br/><a href='".$rjcms_domain."ucenter/active.php?verify=".$token."' target='_blank'>".$rjcms_domain."ucenter/active.php?verify=".$token."</a><br/>如果以上链接无法点击，请将它复制到你的浏览器地址栏中进入访问，该链接24小时内有效。<br/>如果此次激活请求非你本人所发，请忽略本邮件。<br/><p style='text-align:right'>-------- ".$rjcms_name." 敬上</p>";
    ...
    <form action="" method="post" class="form-horizontal form">
        <div class="login-user-name common-div">
            <span class="eamil-icon common-icon">
              <img src="<?php echo $rjcms_domain;?>wap/style/images/eamil.png" />
            </span>
            <input type="text" name="u_name" value="" placeholder="请输入您的手机号" />        
          </div>
          <div class="login-user-pasw common-div">
            <span class="pasw-icon common-icon">
              <img src="<?php echo $rjcms_domain;?>wap/style/images/password.png" />
            </span>
...
<form class="form-horizontal js-ajax-form"  method="post" onsubmit="return chk_form();">
                <div class="weui_cells weui_cells_form">
                    ...
                    <?php if($rjcms_smsname!=""){?><div class="weui_cell weui_vcode">
                        <div class="weui_cell_hd">
