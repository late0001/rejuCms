...
if(!isset($_SESSION['user_name'])){
 ...
 if($u_group<=1){//如果会员组
 alert_href('对不起,您不能观看会员视频，请升级会员！',''.$rjcms_domain.'wap/user.php?op=open');
 } 
}
...
if($d_jifen>0){//积分大于0,普通会员收费
    ...
    $result = mysqli_query($conn, 'select * from rjcms_user where u_name="'.$_SESSION['user_name'].'"');//查询会员积分
	 ...
	 $u_group=$row['u_group'];//到期时间
	     ...
	     if($u_group<=1){//如果会员组
     if($d_jifen>$u_points){
	 alert_href('对不起,您的积分不够，无法观看收费数据，请充值续费！',''.$rjcms_domain.'wap/user.php?op=open');
    }  else{
...
<script type="text/javascript"> 
	...
	$('#shiping_box').html('<div style="text-align:center;width:100%;position: relative;height:210px;background:#000"><?php echo get_ad(1)?><div id="timer"><?php echo $rjcms_miaoshu;?></div></div>');
    //设置延时函数
    function adsUp(){    
        $("#shiping_box").html('<iframe id="video" src="<?php echo $rjcms_domain;?>jx.php?url=<?php $result = mysqli_query($conn, 'select * from rjcms_vod where d_id ='.$d_id.'');
	if (!!$row = mysqli_fetch_array($result)){
$d_scontent=explode("\r\n",$row['d_scontent']);
//print_r($d_scontent);
...
<a href="<?php echo $rjcms_domain;?>wap/addfav.php?title=<?php echo $timu; ?>&dizhi=<?php echo 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING'];?>"><em class="icon icon-48" style="font-size: 19px;"></em></a> </div>
...
{	$d_scontent[$i]=explode('$',$d_scontent[$i]);
if($d_scontent[$i][1]!=""){
	echo'<li><a href="'.$rjcms_domain.'jx.php?url='.get_play($row['d_player']).''.$d_scontent[$i][1].'" target="main" id="'.$d_scontent[$i][0].'">'.$d_scontent[$i][0].'</a></li>';
}
	}
	}else{
		return '';
...
<div id="dashang" style="display: none">  
  <img src="<?php echo $rjcms_dashang; ?>" style="border-radius: 50%;width: 100px;height: 100px">
       ...
       $.modal({    
          ...
          buttons: [
            { text: "打赏", onClick: function(){
               ...
               var url = "<?php echo $rjcms_domain;?>wap/shang.php?fee="+shang_fee;
