...
//判定会员组别
$result = mysqli_query($conn, 'select * from rjcms_user where u_name="'.$_SESSION['user_name'].'"');
...
//获取会员卡信息
$card= mysqli_query($conn, 'select * from rjcms_userka where id="'.$_POST['cardid'].'"');
...
$sql = 'insert into rjcms_user_pay ('.$str[0].') values ('.$str[1].')';
	if(mysqli_query($conn, $sql)){}
...
//判定卡号是否存在
$result = mysqli_query($conn, 'select * from rjcms_user_card where c_number = "'.$_POST['c_number'].'" and c_used=0');
if($row = mysqli_fetch_array($result)){
$day1= $row['c_money'];//天数
if ($row['c_userid']!=""){
...
$sql = 'update rjcms_user_card set '.arrtoupdate($data).' where c_number = "'.$_POST['c_number'].'"';	
if (mysqli_query($conn, $sql)) {
...
<!--页面style css-->
<link rel="stylesheet" href="<?php echo $rjcms_domain;?>wap/weui/weuix.min.css"/>
<link rel="stylesheet" type="text/css" href="<?php echo $rjcms_domain;?>wap/style/css/base.css">
<link rel="stylesheet" type="text/css" href="<?php echo $rjcms_domain;?>wap/style/css/li.css">
<script src="<?php echo $rjcms_domain;?>wap/weui/zepto.min.js"></script>
<script src="<?php echo $rjcms_domain;?>wap/style/js/li.js"></script>
<script type="text/javascript " src="<?php echo $rjcms_domain;?>style/js/history.js "></script>
<title>会员中心-<?php echo $rjcms_seoname;?></title>
<style type="text/css">
...
<section class="touxiang_box">
  <div class="touxiang"> <img src="<?php echo $rjcms_domain;?>wap/style/images/icon_zjongxin_03.png"> </div>
    ...
    <li>
      ...
      <a href="user.php?op=yaoqing" onclick="$('.weui-share').show().addClass('fadeIn');">
        <div class="huiyuan tuijian_y8">
          <p class="clearfix">推广记录<span class="fr"><?php $sql1="SELECT COUNT(*) AS count FROM rjcms_user where u_qid=$u_id"; 
...
$count=$result1['count']; echo $count;?><em class="zhuandao2"></em></span></p>
    ...
    <li>
      <div class="huiyuan jifen" style="background: url(<?php echo $rjcms_domain;?>wap/style/images/shoucang.png) no-repeat 0.33rem center;background-size: 0.31rem 0.29rem">
        <a href="user.php?op=shoucang"><p class="clearfix">我的收藏：<span class="fr"><em class="zhuandao2"></em></span></p></a>
      </div>
    </li>
    <li>
      <div class="huiyuan jifen" style="background: url(<?php echo $rjcms_domain;?>wap/style/images/bangding.png) no-repeat 0.33rem center;background-size: 0.31rem 0.29rem">
    ...
    <div class="weui_panel_bd">
        <a href="javascript:void(0);" class="weui_media_box weui_media_appmsg">
            <div class="weui_media_hd">
                <img class="weui_media_appmsg_thumb circle" src="<?php echo $rjcms_domain;?>wap/style/images/icon_zjongxin_03.png" alt="">
            </div>
...
<?php
							$result = mysqli_query($conn, 'select * from rjcms_userka');
...
<input type="radio" id="profile_gender_0" name="pay" required="required" value="1" checked="">
								...
								<?php if($rjcms_zhifu=1){?><input type="radio" id="profile_gender_1" name="pay" required="required" value="alipay">
...
<div class="weui_panel">
<img src="<?php echo $rjcms_domain;?>wap/style/images/tequan.png" width="100%"> 
</div>
<style type="text/css">
...
<section class="wode_box bgfff">
  <ul>
<?php
                            $result = mysqli_query($conn, 'select * from rjcms_fav where userid="'.$u_id.'"');
							while($row= mysqli_fetch_array($result)){
							?>
    <li>
      <div class="huiyuan jifen">
...
<section class="wode_box bgfff">
我的推广链接：<?php echo $rjcms_domain;?>wap/login.php?op=create&tg=<?php echo $u_id ?>
</section>
...
<section class="wode_box bgfff">
  <ul>
<?php
                            $result = mysqli_query($conn, 'select * from rjcms_user where u_qid="'.$u_id.'"');
							while($row= mysqli_fetch_array($result)){
							?>
    <li>
      <div class="huiyuan jifen">
  ...
  <?php } ?> 
  </ul>我不想邀请，直接购买充值卡充值：<a href="<?php echo $rjcms_qqun;?>" target="_blank">点此购买>>></a>
</section>

<?php }?>
<?php if ($op == 'order'){?>
<?php
									$sql1 = 'select * from rjcms_user_pay where p_uid="'.$row['u_name'].'" order by p_id desc';
...
<input name="buycard" type="button" value="购买会员卡" onclick="window.open('<?php echo $rjcms_qqun;?>')" class="weui_btn weui_btn_primary" style="background-color: #9fc732;border-radius: 50px"/><a>
