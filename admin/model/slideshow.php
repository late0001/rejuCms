...
if (isset($_POST['save'])) {
	...
	$sql = 'insert into rjcms_slideshow (' . $str[0] . ') values (' . $str[1] . ')';
	if (mysqli_query($conn, $sql)) {
		$order = mysql_insert_id();
		if ($_POST['s_order'] == 0) {
			mysqli_query($conn, 'update rjcms_slideshow set s_order = ' . $order . ' where id = ' . $order);
		}
...
if (isset($_GET['del'])) {
	$sql = 'delete from rjcms_slideshow where id = ' . $_GET['del'] . '';
	if (mysqli_query($conn, $sql)) {
