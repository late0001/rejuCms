...
if (isset($_POST['save'])) {
	...
	$result = mysqli_query($conn, 'select * from rjcms_manager where m_name = "' . $_POST['a_name'] . '" and m_id <> ' . $_GET['id'] . '');
	if (mysqli_fetch_array($result)) {
		alert_back('登录帐号重复，请更换登录帐号。');
	}
	$_data['m_name'] = $_POST['a_name'];
	$_data['m_password'] = md5($_POST['a_password']);
	$sql = 'update rjcms_manager set ' . arrtoupdate($_data) . ' where m_id = ' . $_GET['id'] . '';
