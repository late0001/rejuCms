...
if (isset($_POST['save'])) {
	...
	$sql = 'insert into rjcms_nav (' . $str[0] . ') values (' . $str[1] . ')';
	if (mysqli_query($conn, $sql)) {
		$order = mysql_insert_id();
		if ($_POST['order'] == 0) {
			mysqli_query($conn, 'update rjcms_nav set order = ' . $order . ' where id = ' . $order);
		}
...
if (isset($_GET['del'])) {
	$sql = 'delete from rjcms_nav where id = ' . $_GET['del'] . '';
	if (mysqli_query($conn, $sql)) {
