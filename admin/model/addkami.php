...
if (isset($_POST['save'])) {
	...
	for ($i = 0; $i < $num; $i++) {
		$km = getkm($weishu);
		$SQL = "INSERT INTO `rjcms_user_card` (`c_id`, `c_number`, `c_money`, `c_addtime`, `c_pass`, `c_userid`) VALUES (NULL, '{$km}', '{$day}', '{$c_addtime}', '{$card_id}', '{$userid}')";
		mysqli_query($conn, $SQL);
	}
	$sql = "INSERT INTO `rjcms_user_cardclass` (`uniacid`,`title`, `card_id`, `num`, `userid`, `day`) VALUES ('{$uniacid}','{$title}', '{$card_id}', '{$num}', '{$userid}', '{$day}')";
	mysqli_query($conn, $sql);
