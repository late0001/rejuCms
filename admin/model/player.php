...
if (isset($_POST['save'])) {
	...
	$sql = 'insert into rjcms_player (' . $str[0] . ') values (' . $str[1] . ')';
	if (mysqli_query($conn, $sql)) {
		$order = mysql_insert_id();
		if ($_POST['order'] == 0) {
			mysqli_query($conn, 'update rjcms_player set order = ' . $order . ' where id = ' . $order);
		}
...
if (isset($_GET['del'])) {
	$sql = 'delete from rjcms_player where id = ' . $_GET['del'] . '';
	if (mysqli_query($conn, $sql)) {
