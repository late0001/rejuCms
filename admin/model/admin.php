...
if ($_GET['del'] == 1) {
	...
	if (isset($_GET['del'])) {
		$sql = 'delete from rjcms_manager where m_id= ' . $_GET['del'] . '';
...
if (isset($_POST['save'])) {
	...
	$result = mysqli_query($conn, 'select * from rjcms_manager where m_name = "' . $_POST['a_name'] . '"');
	...
	$sql = 'insert into rjcms_manager (' . $str[0] . ') values (' . $str[1] . ')';
	if (mysqli_query($conn, $sql)) {
