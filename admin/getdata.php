...
';
	if (!empty($title)) {
		foreach ($title as $key => $video) {
			...
			$result = mysqli_query($conn, 'select * from rjcms_vod where d_name= "' . $titile . '"');
			if (mysqli_fetch_array($result)) {
				$sql = 'update rjcms_vod set ' . arrtoupdate($_data) . ' where d_name="' . $titile . '"';
				if (mysqli_query($conn, $sql)) {
				}
				echo '<script>alert("影片重复更新最新地址成功!");window.opener=null;window.close();</script>';
			} else {
				$sql = 'insert into rjcms_vod (' . $str[0] . ') values (' . $str[1] . ')';
				if (mysqli_query($conn, $sql)) {
...
if (isset($_POST['execute'])) {
	...
	switch ($_POST['execute_method']) {
		case 'srec':
			$sql = 'update rjcms_vod set d_rec = 1 where d_id in (' . $id . ')';
			break;
		case 'crec':
			$sql = 'update rjcms_vod set d_rec = 0 where d_id in (' . $id . ')';
			break;
		case 'shot':
			$sql = 'update rjcms_vod set d_hot = 1 where d_id in (' . $id . ')';
			break;
		case 'chot':
			$sql = 'update rjcms_vod set d_hot = 0 where d_id in (' . $id . ')';
			break;
		case 'delete':
			$sql = 'delete from rjcms_vod where d_id in (' . $id . ')';
			break;
		default:
...
if (isset($_POST['shift'])) {
	...
	mysqli_query($conn, 'update rjcms_vod set d_parent = ' . $_POST['shift_target'] . ' where d_id in (' . $id . ')');
